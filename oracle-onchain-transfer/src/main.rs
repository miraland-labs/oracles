//! `oracle-onchain-transfer` binary entrypoint.
//!
...
async fn main() -> anyhow::Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            "oracle_onchain_transfer=info,oracle_common=info,tower_http=info".into()
        }))
        .init();
    ...
    info!("oracle-onchain-transfer cluster={cluster:?}");
    ...
    let backend = make_backend(
        ...
        Some(std::path::PathBuf::from(
            "/var/lib/oracle/onchain-transfer/blobs",
        )),
    )
    .await
