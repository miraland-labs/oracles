    ...
    async fn evaluate(
        ...
        if sla.cluster != self.cluster {
            return Ok(finish(
                ...
                None,
                reject(
                    rwa_transfer::TRANSFER_CLUSTER_MISMATCH,
                    "cluster",
                    &format!(
        ...
        let observation = match fetch_observation(ctx.rpc, &evidence.tx_signature).await {
            ...
            Err(FetchError::SignatureFormat(e)) => {
                return Ok(finish(
                    ...
                    None,
                    reject(
                        rwa_transfer::TRANSFER_TX_NOT_FOUND,
                        "tx_signature",
        ...
        if !verdict.approved {
            return Ok(finish(sla, evidence, Some(&observation), verdict));
        }

        if let Err((reason, detail)) = verify_rwa_mint_metadata(ctx.rpc, sla).await {
            return Ok(finish(
                sla,
                evidence,
                Some(&observation),
                reject(reason, "rwa_mint_metadata", &detail),
            ));
        }
        ...
        if !sla.payment_uid.eq_ignore_ascii_case(&want_uid) {
            return Ok(finish(
                ...
                Some(&observation),
                reject(
                    rwa_transfer::TRANSFER_PAYMENT_UID_MISMATCH,
                    "sla.payment_uid",
                    &format!(
        ...
        if let Some(want_nonce) = sla.buyer_nonce.as_deref() {
            ...
            if got.is_empty() || !got.eq_ignore_ascii_case(want_nonce) {
                return Ok(finish(
                    ...
                    Some(&observation),
                    reject(
                        rwa_transfer::TRANSFER_BUYER_NONCE_MISMATCH,
                        "buyer_nonce",
                        &if got.is_empty() {
        ...
        // Wave A §2.2.1 cross-payment replay refusal
                ...
                .await
            {
                Ok(true) => {
                    return Ok(finish(
                        ...
                        Some(&observation),
                        reject(
                            rwa_transfer::TRANSFER_TX_SIGNATURE_REUSED,
                            "tx_signature",
                            &format!(
...
pub fn verify_observed_transfer(
    ...
    if observation.failed {
        return reject(
            rwa_transfer::TRANSFER_TX_FAILED,
            "tx_status",
    ...
    if let Some(created_at) = payment_created_at {
        match observation.block_time {
            ...
            Some(bt) if bt < created_at => {
                return reject(
                    rwa_transfer::TRANSFER_EVIDENCE_PREDATES_PAYMENT,
                    "block_time",
                    &format!(
    ...
    for (idx, expected) in sla.expected_transfers.iter().enumerate() {
        ...
        let _ = rwa_transfer::TRANSFER_RECIPIENT_NOT_RESOLVABLE;
        ...
        if !direction_ok {
            return reject(
                rwa_transfer::TRANSFER_DIRECTION_MISMATCH,
                &format!("expected_transfer[{idx}]"),
                &format!(
        ...
        if abs_delta < min_amount {
            return reject(
                rwa_transfer::TRANSFER_AMOUNT_INSUFFICIENT,
                &format!("expected_transfer[{idx}]"),
        ...
        if let Some(sender_owner) = expected.sender_owner.as_deref() {
            ...
            let sender_pre_amount = match sender_pre {
                ...
                None => {
                    return reject(
                        rwa_transfer::TRANSFER_SENDER_MISMATCH,
                        &format!("expected_transfer[{idx}]"),
                        &format!(
...
/// Attach normative resolution-envelope `details` per rwa-transfer §7.
fn finish(
    sla: &TransferSla,
    evidence: &TransferEvidence,
...
fn build_resolution_details(
    ...
    json!({
        ...
        "cluster": cluster,
        "tokenProgram": sla.token_program,
        "transferHookProgram": sla.transfer_hook_program,
        "offeringId": sla.offering_id,
...
// ---------------------------------------------------------------------------
// RWA mint metadata (token program pin)
// ---------------------------------------------------------------------------

async fn verify_rwa_mint_metadata(
...
pub(crate) fn verify_rwa_mint_from_account(
    mint_owner: &Pubkey,
    mint_data: &[u8],
    expected_token_program: &Pubkey,
    sla_transfer_hook_program: Option<&str>,
) -> Result<(), (u16, String)> {
    if mint_owner != expected_token_program {
        return Err((
            rwa_transfer::RWA_TOKEN_PROGRAM_MISMATCH,
            format!(
                "mint owner {} != SLA token_program {}",
                mint_owner, expected_token_program
            ),
        ));
    }

    if *expected_token_program == spl_token::ID {
        if sla_transfer_hook_program.is_some() {
            return Err((
                rwa_transfer::RWA_TRANSFER_HOOK_MISMATCH,
                "SLA declares transfer_hook_program but mint owner is classic SPL Token".into(),
            ));
        }
        return Ok(());
    }

    let on_chain_hook = parse_mint_transfer_hook_program(mint_data)?;
    verify_transfer_hook_against_sla(on_chain_hook, sla_transfer_hook_program)
}

fn parse_mint_transfer_hook_program(mint_data: &[u8]) -> Result<Option<Pubkey>, (u16, String)> {
    let mint = StateWithExtensions::<Mint>::unpack(mint_data).map_err(|e| {
        (
            rwa_transfer::RWA_TRANSFER_HOOK_MISMATCH,
            format!("Token-2022 mint unpack failed: {e}"),
        )
    })?;
...
fn verify_transfer_hook_against_sla(
    on_chain_hook: Option<Pubkey>,
    sla_transfer_hook_program: Option<&str>,
) -> Result<(), (u16, String)> {
    match (on_chain_hook, sla_transfer_hook_program) {
        (Some(on_chain), Some(sla_str)) => {
            let expected = Pubkey::from_str(sla_str).map_err(|e| {
                (
                    rwa_transfer::RWA_TRANSFER_HOOK_MISMATCH,
                    format!("invalid transfer_hook_program in SLA: {e}"),
                )
            })?;
            if on_chain != expected {
                return Err((
                    rwa_transfer::RWA_TRANSFER_HOOK_MISMATCH,
                    format!(
                        "mint transfer hook program {} != SLA transfer_hook_program {}",
                        on_chain, expected
                    ),
                ));
            }
        ...
        (Some(on_chain), None) => Err((
            rwa_transfer::RWA_TRANSFER_HOOK_MISMATCH,
            format!(
                "mint has Transfer Hook extension (program {}) but SLA omits transfer_hook_program",
                on_chain
            ),
        )),
        (None, Some(_)) => Err((
            rwa_transfer::RWA_TRANSFER_HOOK_MISMATCH,
            "SLA declares transfer_hook_program but mint has no Transfer Hook extension".into(),
        )),
        (None, None) => Ok(()),
    }
}
...
mod tests {
    ...
    fn rejects_when_mint_mismatch() {
        ...
        let r = verify_observed_transfer(&sla, &observation, None);
        assert!(!r.approved);
        assert_eq!(r.resolution_reason, rwa_transfer::TRANSFER_MINT_MISMATCH);
    }
    ...
    fn rejects_when_meta_err_set() {
        ...
        let r = verify_observed_transfer(&sla, &observation, None);
        assert!(!r.approved);
        assert_eq!(r.resolution_reason, rwa_transfer::TRANSFER_TX_FAILED);
    }
    ...
    fn replay_refusal_uses_correct_resolution_code() {
        ...
        assert_eq!(rwa_transfer::TRANSFER_TX_SIGNATURE_REUSED, 459);
    }
    ...
    fn payment_uid_mismatch_constant_in_range() {
        assert_eq!(rwa_transfer::TRANSFER_PAYMENT_UID_MISMATCH, 460);
        assert!(rwa_transfer::RANGE.contains(&rwa_transfer::TRANSFER_PAYMENT_UID_MISMATCH));
    }

    #[test]
    fn buyer_nonce_mismatch_constant_in_range() {
        assert_eq!(rwa_transfer::TRANSFER_BUYER_NONCE_MISMATCH, 461);
        assert!(rwa_transfer::RANGE.contains(&rwa_transfer::TRANSFER_BUYER_NONCE_MISMATCH));
    }
    ...
    fn sender_set_missing_pre_row_rejects() {
        ...
        let r = verify_observed_transfer(&sla, &observation, None);
        assert!(!r.approved);
        assert_eq!(r.resolution_reason, rwa_transfer::TRANSFER_SENDER_MISMATCH);
    ...
    fn sender_mismatch_constant_in_range() {
        assert_eq!(rwa_transfer::TRANSFER_SENDER_MISMATCH, 457);
        assert!(rwa_transfer::RANGE.contains(&rwa_transfer::TRANSFER_SENDER_MISMATCH));
    }

    #[test]
    fn transfer_hook_mismatch_constant_in_range() {
        assert_eq!(rwa_transfer::RWA_TRANSFER_HOOK_MISMATCH, 449);
        assert!(rwa_transfer::RANGE.contains(&rwa_transfer::RWA_TRANSFER_HOOK_MISMATCH));
    }

    #[test]
    fn rwa_mint_rejects_token_program_owner_mismatch() {
        let token_2022 = Pubkey::from_str(TOKEN_2022_PROGRAM).unwrap();
        let classic = spl_token::ID;
        let err = verify_rwa_mint_from_account(&classic, &[], &token_2022, None).unwrap_err();
        assert_eq!(err.0, rwa_transfer::RWA_TOKEN_PROGRAM_MISMATCH);
    }

    #[test]
    fn rwa_mint_rejects_hook_on_classic_spl_owner() {
        let classic = spl_token::ID;
        let hook = Pubkey::new_unique();
        let err = verify_rwa_mint_from_account(&classic, &[], &classic, Some(&hook.to_string()))
            .unwrap_err();
        assert_eq!(err.0, rwa_transfer::RWA_TRANSFER_HOOK_MISMATCH);
    }

    #[test]
    fn transfer_hook_pin_accepts_matching_program() {
        let hook = Pubkey::new_unique();
        verify_transfer_hook_against_sla(Some(hook), Some(&hook.to_string())).unwrap();
    }

    #[test]
    fn transfer_hook_pin_rejects_sla_without_on_chain_extension() {
        let hook = Pubkey::new_unique();
        let err = verify_transfer_hook_against_sla(None, Some(&hook.to_string())).unwrap_err();
        assert_eq!(err.0, rwa_transfer::RWA_TRANSFER_HOOK_MISMATCH);
    }

    #[test]
    fn transfer_hook_pin_rejects_on_chain_without_sla_field() {
        let hook = Pubkey::new_unique();
        let err = verify_transfer_hook_against_sla(Some(hook), None).unwrap_err();
        assert_eq!(err.0, rwa_transfer::RWA_TRANSFER_HOOK_MISMATCH);
    }

    #[test]
    fn transfer_hook_pin_rejects_program_mismatch() {
        let on_chain = Pubkey::new_unique();
        let sla = Pubkey::new_unique();
        let err =
            verify_transfer_hook_against_sla(Some(on_chain), Some(&sla.to_string())).unwrap_err();
        assert_eq!(err.0, rwa_transfer::RWA_TRANSFER_HOOK_MISMATCH);
    }
    ...
    proptest! {
        ...
        fn p_ot_4_amount_gating(
            ...
            if observed >= min_amount {
                ...
                prop_assert!(
                    reason == rwa_transfer::TRANSFER_AMOUNT_INSUFFICIENT
                        || reason == rwa_transfer::TRANSFER_DIRECTION_MISMATCH,
