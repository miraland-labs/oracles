...
| `forge_listing_id`          | `string`              | yes      | Forge listing identity, exactly as Forge publishes it. Used verbatim as the `{listing_id}` path segment and signed-message field on the verdict door (`docs/AGENT_API.md` "Oracle verdict (escrow only)"). Distinct from `payment_uid`; never derived from or replaced by it. |
