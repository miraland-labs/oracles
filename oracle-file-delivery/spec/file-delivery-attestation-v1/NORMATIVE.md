...
| `listing_id`                | `string` (UUID)       | yes      | The Forge listing identity exactly as Forge publishes it (`GET /api/v1/listings/{id}`). Distinct from `payment_uid`: this is the listing being judged, never derived from or replaced by the payment identifier. Flows into the verdict request's URL path and signed oracle message per the pinned `http402-forge-api` contract (`docs/ESCROW_TWO_DOORS.md` §4). |
| `payment_uid`               | hex `string` (64)     | yes      | Hex-encoded 32-byte `payment_uid` from the on-chain `Payment` this SLA is bound to. A payment identifier, not a listing identity.                                       |
