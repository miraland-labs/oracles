...
pub struct FileDeliverySla {
    ...
    /// (the listing UUID from `GET /api/v1/listings/{id}`) — the listing this
    /// SLA is judging delivery for. Distinct from `payment_uid`: this is the
    /// listing being judged, never derived from or replaced by the payment
    /// identifier. Flows into the verdict request's URL path
    /// (`GET /api/v1/oracle/listings/{listing_id}/artifact`) and into the
    /// signed oracle message
    /// (`forge-oracle-v1|{listing_id}|{payment_uid_hex}|{ts}|{host}`) exactly
    /// as published in the pinned `http402-forge-api` contract
